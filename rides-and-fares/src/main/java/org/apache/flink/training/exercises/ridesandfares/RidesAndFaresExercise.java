...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        private transient ValueState<TaxiRide> outRide;
        private transient ValueState<TaxiFare> outFare;

        @Override
        public void open(Configuration config) {
            outRide =
                    getRuntimeContext()
                            .getState(new ValueStateDescriptor<>("outRide", TaxiRide.class));
            outFare =
                    getRuntimeContext()
                            .getState(new ValueStateDescriptor<>("outFare", TaxiFare.class));
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            TaxiFare fare = outFare.value();
            if (fare != null) {
